...
def test_read_user_settings_file_not_found():
    with pytest.raises(KeyError):
...
def test_sorted_by_date_empty_df():
    """Тест пустой таблицы"""
...
def test_cards_info_single_card():
    df = pd.DataFrame([
        {"Номер карты": "*1234", "Сумма платежа": -100.0},
        {"Номер карты": "*1234", "Сумма платежа": -200.0}
    ])
...
def test_cards_info_large_sums():
    df = pd.DataFrame([
        {"Номер карты": "*1234", "Сумма платежа": -10000.0},
        {"Номер карты": "*5678", "Сумма платежа": -20000.0}
    ])
    expected_result = [
...
def test_top_transactions_no_expenditures():
